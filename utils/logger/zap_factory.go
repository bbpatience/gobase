...
func CreateZapFactory(entry func(zapcore.Entry) error) *zap.Logger {
	...
	appDebug := variables.ConfigYml.GetBool("AppDebug")
	...
	timePrecision := variables.ConfigYml.GetString("Logs.TimePrecision")
	...
	switch variables.ConfigYml.GetString("Logs.TextFormat") {
	case "console":
	...
	//写入器
	fileName := variables.ConfigYml.GetString("Logs.GoSkeletonLogName")
	lumberJackLogger := &lumberjack.Logger{
		Filename:   fileName,                                      //日志文件的位置
		MaxSize:    variables.ConfigYml.GetInt("Logs.MaxSize"),    //在进行切割之前，日志文件的最大大小（以MB为单位）
		MaxBackups: variables.ConfigYml.GetInt("Logs.MaxBackups"), //保留旧文件的最大个数
		MaxAge:     variables.ConfigYml.GetInt("Logs.MaxAge"),     //保留旧文件的最大天数
		Compress:   variables.ConfigYml.GetBool("Logs.Compress"),  //是否压缩/归档旧文件
	}
